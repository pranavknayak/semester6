...
void configure_server_context(SSL_CTX *ctx) {
  ...
  if (SSL_CTX_use_certificate_file(ctx, "./task1/certificates/root.crt",
                                   SSL_FILETYPE_PEM) <= 0) {
    ERR_print_errors_fp(stderr);
    exit(EXIT_FAILURE);
  }

  if (SSL_CTX_use_PrivateKey_file(ctx, "./task1/keys/rootKey.pem",
                                  SSL_FILETYPE_PEM) <= 0) {
    ERR_print_errors_fp(stderr);
    exit(EXIT_FAILURE);
  }
...
void configure_client_context(SSL_CTX *ctx) {
   ...
   * trust it directly.
   */
  if (!SSL_CTX_load_verify_locations(ctx, "./task1/certificates/root.crt",
                                     NULL)) {
    ERR_print_errors_fp(stderr);
    exit(EXIT_FAILURE);
  }
